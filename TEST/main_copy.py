...
def main():
    ...
    while True:
        ...
        if stato_gioco == "MENU_PRINCIPALE":
            ...
            # Definizione rettangoli dei pulsanti
            # Posizione di "GIOCA" spostata dove prima c'era "IMPOSTAZIONI"
            rect_gioca = pygame.Rect(cx, ALTEZZA // 2 + 160, larghezza_btn, altezza_btn)
            rect_uscita = pygame.Rect(cx, ALTEZZA // 2 + 270, larghezza_btn, altezza_btn)

            # Sfondo menu
            screen.blit(img_menu, (0, 0))

            # Pulsante GIOCA (verde) con hover
            ...
            screen.blit(
                txt_gioca,
                (rect_gioca.centerx - txt_gioca.get_width() // 2, rect_gioca.centery - txt_gioca.get_height() // 2)
            )

            # Pulsante USCITA (rosso) con hover
            ...
            screen.blit(
                txt_uscita,
                (rect_uscita.centerx - txt_uscita.get_width() // 2, rect_uscita.centery - txt_uscita.get_height() // 2)
            )
            ...
            for e in event:
                if e.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    elif rect_uscita.collidepoint(e.pos):
                        pygame.quit()
                        sys.exit()
        ...
        # --- SELEZIONE PERSONAGGIO ---              
            ...
            # Posizione più bassa del personaggio
            personaggio_y = ALTEZZA // 2 - 40

            if frames_correnti:
                # Anima la GIF: ogni 5 tick cambia frame
                frame_da_mostrare = frames_correnti[(indice_frame_personaggio // 5) % len(frames_correnti)]
                rect_gif = frame_da_mostrare.get_rect(center=(LARGHEZZA // 2, personaggio_y))
                ...
                rect_placeholder = pygame.Rect(LARGHEZZA // 2 - 100, personaggio_y - 100, 200, 200)
                pygame.draw.rect(screen, (200, 200, 200), rect_placeholder, border_radius=20)

            # Nome del personaggio con rettangolo bianco dietro
            txt_nome = font.render(personaggio_corrente["nome"], True, (0, 0, 0))
            txt_x = LARGHEZZA // 2 - txt_nome.get_width() // 2
            txt_y = 180

            # Rettangolo bianco dietro il nome
            rect_nome_bg = pygame.Rect(txt_x - 10, txt_y - 5, txt_nome.get_width() + 20, txt_nome.get_height() + 10)
            pygame.draw.rect(screen, (255, 255, 255), rect_nome_bg)

            # Disegno il testo sopra il rettangolo
            screen.blit(txt_nome, (txt_x, txt_y))
        ...
        # --- SELEZIONE LIVELLO ---
            ...
            # Anteprime invertite (2 e 3)
            previews = [preview1, preview3, preview2]
            # Mappa per collegare indice visuale con indice reale dei livelli
            mappa_indici = [0, 2, 1]

            # Font per le scritte dei livelli
            font_label = pygame.font.Font(None, 50) 
            ...
            for i, pv in enumerate(previews):
                ...
                # Label sopra l'immagine (più alta e più piccola)
                label_text = f"LIVELLO {i+1}"
                txt = font_label.render(label_text, True, (0, 0, 0))  # Testo nero
                txt_rect = txt.get_rect(center=(x + pv_w // 2, y - 30))  # rialzata sopra anteprima

                # Rettangolo bianco dietro la scritta
                shadow_rect = pygame.Rect(
                    txt_rect.left - 6, txt_rect.top - 2, txt_rect.width + 12, txt_rect.height + 4
                )
                shadow_surf = pygame.Surface((shadow_rect.width, shadow_rect.height))
                shadow_surf.fill((255, 255, 255))  # Bianco pieno
                screen.blit(shadow_surf, (shadow_rect.x, shadow_rect.y))

                # Disegno scritta sopra il rettangolo
                ...
                if e.type == pygame.MOUSEBUTTONDOWN:
                    for i, r in enumerate(rects):
                        if r.collidepoint(e.pos):
                            livello_scelto = livelli_possibili[mappa_indici[i]]
